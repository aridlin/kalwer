...
inline constexpr std::array commands = {
    ...
    Command{"/pvz", "Garden Defense: plants, zombies and waves", ""},
...
inline std::vector<const Command*> matching_commands(std::string prefix) {
    ...
    static const Command unlock{"/unlockall","Unlock games",""};
    if(prefix=="/unlockall"){matches.push_back(&unlock);return matches;}
