...
struct Breakout {
    ...
    void reset(unsigned){paddle=210;score=board=0;lives=3;time=flash=0;started=over=won=false;layout();}
    void key(int k){if(over)return;started=true;if(k==1 || k=='a')paddle=std::max(58.,paddle-25);if(k==2 || k=='d')paddle=std::min(362.,paddle+25);if(k==' ' || k==13)flying=true;}
    void pointer(double x,double,int button){if(over)return;paddle=std::clamp(x,58.,362.);started=true;if(button==1)flying=true;}
    void advance(double dt){if(!started || over)return;time+=dt;flash=std::max(0.,flash-dt);if(!flying){ball_x=paddle;return;}
        for(int step=0;step<8;step++){double h=dt/8,ox=ball_x,oy=ball_y;ball_x+=vx*h;ball_y+=vy*h;
            ...
            if(vy>0 && oy<=397 && ball_y>=397 && std::abs(ball_x-paddle)<=44){double angle=std::clamp((ball_x-paddle)/44.,-.95,.95)*1.12;double speed=std::min(430.,270.+board*45+score*.07);vx=std::sin(angle)*speed;vy=-std::cos(angle)*speed;ball_y=397;}
    ...
    template<class P>void draw(P& p)const {
        ...
        p.rect(paddle-40,404,80,10,0x8bdcff);p.circle(ball_x,ball_y,6,0xffd579);
