...
inline std::string import_wad(std::filesystem::path source) {
    ...
    return ec?"Could not copy the WAD.":"Installed "+source.filename().string()+". Open /koom and select it with Left/Right.";
}
struct Session {
...
#endif
    std::thread thread;
    explicit Session(std::filesystem::path wad):thread([this,wad]{run(wad);}){}
    ~Session(){
    ...
    void run(std::filesystem::path wad) {
#ifndef _WIN32
        ...
        auto base=dir/"freedoom2.wad";bool is_base=false;
        ...
        std::wstring command=quote(executable)+L" -iwad "+quote(base)+L" -config "+quote(config)+L" -nosound";
        ...
        std::vector<std::string> args{executable.string(),"-iwad",base.string(),"-config",config.string(),"-nosound"};
...
struct Game {
    std::shared_ptr<Session> session;std::vector<std::filesystem::path> wads;int selected=0;uint32_t rewarded_maps=0;
    std::array<unsigned char,256> keys{};
    void reset(){session.reset();rewarded_maps=0;keys.fill(0);wads.clear();wads.push_back({});std::error_code ec;
        ...
        std::sort(wads.begin()+1,wads.end());selected=std::clamp(selected,0,int(wads.size())-1);
    }
    void key(int k,bool down=true){
        if(!session){if(down && k==1)selected=(selected+int(wads.size())-1)%wads.size();if(down && k==2)selected=(selected+1)%wads.size();if(down && (k==13 || k==' '))session=std::make_shared<Session>(wads[selected]);return;}
    ...
    template<class P>void draw(P& p)const {
        if(!session){p.text(28,100,23,"KOOM",0x8ce9b3);p.text(28,150,14,wads.empty() || wads[selected].empty()?"Freedoom: Phase 2 (included)":wads[selected].filename().string(),0xe0f5e8);p.text(28,195,12,"Left / Right: choose WAD   Enter: play",0x8dada1);p.text(28,235,12,"/wad-import <path> installs your own WAD",0x8dada1);return;}
