...
int main(){
    ...
    assert(atomic_text(wallet.path,"500 7\n"));wallet.load();assert(wallet.balance==500 && wallet.wins==7 && !wallet.owned);
    games::Shop shop;shop.selected=2;shop.activate();assert(wallet.balance==410 && wallet.has(2) && wallet.uses(2) && wallet.wins==7);
    shop.activate();assert(wallet.balance==410 && !wallet.uses(2));shop.activate();assert(wallet.balance==410 && wallet.uses(2));
    Wallet restored;restored.path=wallet.path;restored.load();assert(restored.balance==410 && restored.uses(2));
    shop.selected=1;shop.activate();assert(wallet.balance==160 && wallet.uses(1));shop.selected=3;shop.activate();assert(wallet.balance==160 && !wallet.has(3));
    ...
    assert(wallet.credit(100));assert(wallet.has(1) && wallet.has(2));shop.selected=3;shop.activate();games::Game peggle(games::Kind::peggle,1);assert(peggle.arcade.layout==4 && peggle.arcade.pegs.size()>30);
